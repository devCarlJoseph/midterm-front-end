...
const storeCategories: Category[] = [
  ...
  {
    id: 6,
    categoryName: "Meat Shops",
    icon: <Beef size={16} />,
  },
  {
    id: 7,
    categoryName: "Seafood Markets",
    icon: <Fish size={16} />,
  },
  {
    id: 8,
...
const generateStores = (): StoreItem[] => {
  const stores: StoreItem[] = [];

  Object.entries(storesByCategory).forEach(
    ([category, storeNames]) => {
      storeNames.forEach((name, index) => {
        stores.push({
          ...
          image: `https://placehold.co/600x350/e8f3ed/237a55?text=${encodeURIComponent(
            name
          )}`,
        });
      });
